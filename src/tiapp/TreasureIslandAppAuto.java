...
public class TreasureIslandAppAuto implements Runnable {
	...
	public TreasureIslandAppAuto(Paxos paxos, Logger logger, String gameId, int numPlayers, int yourPlayer) {
		...
		try {
			...
			logger.log(Level.WARNING,
					"Encountered SecurityException while trying to access the environment variable UPDATEDISPLAY.", se);
		}
	...
	private static class AutoMoveGenerator {
		...
		public AutoMoveGenerator(int maxmoves, int interval, String randseed, String failmode, Logger logger) {
			...
			logger.info("AutoMoveGenerator setup maxmoves = " + this.maxmoves + ", interval = " + this.interval
					+ ", randseed = " + randseed + ", failmode = " + this.failmode);
		...
		public String nextMove() {
			...
			if (maxmoves < 10) // for very small maxmoves, we do not want to fail quickly. So pick a larger
								// number.
	...
	public static void main(String args[]) throws IOException, InterruptedException {
		if (args.length < 5) {
			System.err.println(
					"Usage: java comp512st.tiapp.TreasureIslandAppAuto processhost:port processhost:port,processhost:port,... gameid numplayers playernum maxmoves interval randseed [FAILMODE]");
			System.exit(1);
		}
		...
		try {
			String logFile = String.format("logs/%s-%s-%s.log",
					args[2], // gameid
					args[0].replace(':', '.'), // myProcess address, safe for filenames
					args[4] // player number
			);
